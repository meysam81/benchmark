...
get_list() ->
    [
     %% == comn types
     #{code => 1,
       name => 'my_app.sample.type.Pong',
       codec => my_app_sample_type,
       actor => undefined},

     #{code => 2,
       name => 'my_app.sample.type.FactResp',
       codec => my_app_sample_type,
       actor => undefined},

     %% == comn functions
     #{code => 51,
       name => 'my_app.sample.func.Ping',
       codec => my_app_sample_func,
       actor => my_app_ingw_comn},

     #{code => 52,
       name => 'my_app.sample.func.Fact',
       codec => my_app_sample_func,
       actor => my_app_ingw_comn}
    ].
